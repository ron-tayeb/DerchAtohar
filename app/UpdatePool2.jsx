...
export default function UpdatePool2({ navigation, route }) {
    ...
    useEffect(() => { 
        const unsubscribe = navigation.addListener('focus',async () => {//מאזין כל פעם שהוא נכנס לדף ומפעיל את הפונקציות הנבחרות
            let pool = route.params.item
            console.log(`pool`, pool)
            setName(pool.Name)
            setopemimgHours(JSON.stringify(pool.opemimgHours))
            setCity(pool.District)
            let p = JSON.stringify(pool.Payment)
            setPrice(p)
            let c = JSON.stringify(pool.PhoneNumber)
            setPhone(c)
            setAddress(pool.Address)
            setPoolCode(pool.poolCode)
    ...
    const addProduct = async () => {
        if (checkDeitles()) {
            ...
            console.log(`product`, pool)
            setModalLoadVisible(!modalLoadVisible)
            await fetch(`${ServerApi()}/api/UpdatePool`, {
                method: 'POST',
                ...
                .then((result) => {
                    ...
                    if (result.isOk || result === "order update") {
                    ...
                    <TouchableOpacity onPress={() => { addProduct() }}>
                        <View style={[styles.button, { marginTop: 30, }]} >
                            <LinearGradient
                                ...
                                <Text style={[styles.textlogin, { color: "#fff" }]}>          עדכן בריכה          </Text>
                            </LinearGradient >
                        </View>
                    </TouchableOpacity>
